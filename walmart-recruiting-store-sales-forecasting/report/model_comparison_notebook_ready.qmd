...
def _coerce_markdown_series(series: pd.Series) -> pd.Series:
    if series.dtype == bool:
        return series.astype(float)
    as_str = series.astype(str).str.strip().str.upper()
    mapped = as_str.map({"TRUE": 1.0, "FALSE": 0.0, "NA": np.nan, "NAN": np.nan, "": np.nan})
    numeric = pd.to_numeric(series, errors="coerce")
    if numeric.notna().sum() == 0 and mapped.notna().sum() > 0:
        return mapped.astype(float)
    out = numeric.astype(float)
    mask = out.isna() & mapped.notna()
    out.loc[mask] = mapped.loc[mask]
    return out


def ensure_prepared_features(root: Path = PROJECT_ROOT) -> tuple[Path, Path]:
    train_feat_path = root / "train_feat.parquet"
    test_feat_path = root / "test_feat.parquet"
    ...
    for frame in (train, test, features):
        if "IsHoliday" in frame.columns:
            frame["IsHoliday"] = (
                ...
                .fillna(frame["IsHoliday"])
            )

    for col in ["MarkDown1", "MarkDown2", "MarkDown3", "MarkDown4", "MarkDown5"]:
        if col in features.columns:
            features[col] = _coerce_markdown_series(features[col])

    train_feat = train.merge(features, on=["Store", "Date", "IsHoliday"], how="left")
    train_feat = train_feat.merge(stores, on=["Store"], how="left")
    test_feat = test.merge(features, on=["Store", "Date", "IsHoliday"], how="left")
...
train_feat_path, test_feat_path = ensure_prepared_features(PROJECT_ROOT)
...
phase2_artifacts = run_phase2(PHASE2_BEST_CFG)
phase2_artifacts
```
